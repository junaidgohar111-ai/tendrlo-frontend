  ...
  return (
    <main className="min-h-screen overflow-hidden">
      ...
      <section className="relative bp-grid border-b border-blueprint-100 overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-96 h-96 bg-blueprint-500/5 rounded-full blur-3xl animate-pulse" style={{animationDuration:'4s'}}/>
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-blueprint-500/5 rounded-full blur-3xl animate-pulse" style={{animationDuration:'6s'}}/>

        <div ref={hero.ref} className="mx-auto max-w-7xl px-4 sm:px-6 py-12 sm:py-20">
          <div className="max-w-3xl">
            <img
              src={LOGO}
              alt="Tendrlo"
              className={`h-12 sm:h-16 w-auto mb-6 transition-all duration-700 ${hero.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
            />
            <span className={`inline-block rounded-full border border-blueprint-100 bg-blueprint-50 px-3 py-1 text-xs font-mono uppercase tracking-wider text-blueprint-600 transition-all duration-700 delay-100 ${hero.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
              B2B Tendering Marketplace
            </span>
            <h1 className={`mt-4 font-display text-3xl sm:text-5xl md:text-6xl font-semibold tracking-tight leading-tight transition-all duration-700 delay-200 ${hero.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
              Post the scope.<br/>
              <span className="text-blueprint-500">Let contractors</span><br/>
              bid for it.
            </h1>
            <p className={`mt-4 text-base sm:text-lg text-slate-500 max-w-xl leading-relaxed transition-all duration-700 delay-300 ${hero.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <div className={`mt-8 flex flex-col sm:flex-row gap-3 transition-all duration-700 delay-400 ${hero.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            {/* Animated stats */}
            <dl className={`mt-12 grid grid-cols-3 gap-4 max-w-sm font-mono transition-all duration-700 delay-500 ${hero.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
              {[
                { value: 27, suffix: '+', label: 'Categories' },
                { value: 0, suffix: '$', label: 'Listing fee', prefix: '$' },
                { label: 'Bid review', custom: 'Free' },
              ].map((s, i) => (
                <div key={i} className="group">
                  <dt className="text-xs text-slate-500 uppercase tracking-wide">{s.label}</dt>
                  <dd className="text-xl sm:text-2xl font-semibold mt-1 text-ink group-hover:text-blueprint-500 transition-colors">
                    {s.custom ? s.custom : <><CountUp target={s.value!} suffix={s.suffix}/></>}
                  </dd>
                </div>
              ))}
            </dl>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <Link key={i} href="/projects"
              className="relative rounded-card overflow-hidden h-40 sm:h-56 group cursor-pointer">
      ...
      <section ref={cta.ref} className="mx-auto max-w-7xl px-4 sm:px-6 py-12 sm:py-16">
        <div className={`relative rounded-card bg-ink text-white p-8 sm:p-12 overflow-hidden transition-all duration-700 ${cta.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="absolute top-0 right-0 w-64 h-64 bg-blueprint-500/10 rounded-full blur-3xl"/>
          <div className="absolute bottom-0 left-0 w-48 h-48 bg-blueprint-500/10 rounded-full blur-3xl"/>
