...
export default function MessagesPage() {
  ...
  useEffect(()=>{ api('/messages/conversations').then(d=>setConvs(d.conversations)).catch(console.error).finally(()=>setLoading(false)); },[]);

  function timeAgo(d:string){ const m=Math.floor((Date.now()-new Date(d).getTime())/60000); if(m<1)return'just now'; if(m<60)return`${m}m`; const h=Math.floor(m/60); if(h<24)return`${h}h`; return new Date(d).toLocaleDateString(); }
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="mx-auto max-w-2xl px-4 sm:px-6 py-8">
        <h1 className="font-display text-2xl font-semibold mb-5">Messages</h1>
        ...
        {!loading&&convs.length===0&&(
          <div className="text-center py-16 rounded-card border border-blueprint-100">
            ...
            <p className="text-slate-500 text-sm mt-1">Conversations start when you contact a contractor about a project.</p>
            <Link href="/projects" className="mt-4 inline-block text-sm text-blueprint-600 font-medium hover:underline">Browse projects</Link>
          </div>
        )}
        <div className="space-y-2">
          {convs.map((c:any)=>(
            <Link key={c.id} href={`/messages/${c.id}`} className="flex items-center gap-4 rounded-card border border-blueprint-100 bg-white p-4 hover:border-blueprint-500 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {c.last_message_at&&<span className="text-xs text-slate-400 font-mono shrink-0">{timeAgo(c.last_message_at)}</span>}
                </div>
                <p className="text-xs text-slate-400 truncate">{c.project_title}</p>
                {c.last_message&&<p className="text-sm text-slate-500 truncate mt-0.5">{c.last_message}</p>}
              </div>
              {Number(c.unread_count)>0&&(
                <span className="shrink-0 w-5 h-5 rounded-full bg-blueprint-500 text-white text-xs flex items-center justify-center font-mono">{c.unread_count}</span>
              )}
            </Link>
          ))}
        </div>
