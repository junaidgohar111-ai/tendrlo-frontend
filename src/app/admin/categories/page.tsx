...
export default function AdminCategories() {
  ...
  return (
    <div className="p-6 sm:p-8">
      ...
      <p className="text-slate-500 text-sm mb-6">Add new categories here — they appear everywhere instantly.</p>
      <div className="rounded-card border border-blueprint-100 bg-white p-5 mb-6">
        ...
        <form onSubmit={add} className="flex gap-3">
          <input
            ...
            required
            placeholder="e.g. Food and Catering, Raw Materials..."
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-blueprint-100">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-slate-500 text-xs uppercase">Slug</th>
              ...
              <tr key={c.id} className="hover:bg-slate-50">
                <td className="px-4 py-3">
                  {editId === c.id ? (
                    <input
                      ...
                      className="rounded-card border border-blueprint-500 px-3 py-1.5 text-sm focus:outline-none"
                    />
                  ) : (
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    {editId === c.id ? (
                      <>
                        <button onClick={() => save(c.id)} className="text-xs px-2.5 py-1.5 rounded-card border border-emerald-100 text-emerald-600 hover:bg-emerald-50">Save</button>
                      ...
                      <>
                        <button onClick={() => { setEditId(c.id); setEditName(c.name); }} className="text-xs px-2.5 py-1.5 rounded-card border border-blueprint-100 text-blueprint-600 hover:bg-blueprint-50">Edit</button>
                        <button onClick={() => del(c.id, c.name)} className="text-xs px-2.5 py-1.5 rounded-card border border-red-100 text-red-600 hover:bg-red-50">Delete</button>
                      </>
                    )}
                  </div>
                </td>
