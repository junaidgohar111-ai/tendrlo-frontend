      ...
      <div className="rounded-card border border-blueprint-100 bg-white overflow-hidden overflow-x-auto">
        <table className="w-full text-sm min-w-[700px]">
          <thead className="bg-slate-50 border-b border-blueprint-100">
            <tr>{['Name','Email','Role','Joined','Status','Actions'].map(h=><th key={h} className="text-left px-4 py-3 font-medium text-slate-500 text-xs uppercase">{h}</th>)}</tr>
          </thead>
              ...
              <tr key={u.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    ...
                    {u.role !== 'admin' && (
                      <button onClick={()=>deleteUser(u.id, u.full_name)} className="text-xs px-3 py-1.5 rounded-card border border-red-200 text-red-700 hover:bg-red-50 transition-colors">
                        Delete
                      </button>
                    )}
                  </div>
